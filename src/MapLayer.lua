...
function MapLayer:win()
    ...
    function createWinLayer()
        ...
        local bestScore = cc.Label:createWithTTF(G.formatTime(best),"res/font/BradleyHandITC.TTF",38)
        bestScore:setColor(cc.c3b(113,140,142))
        bestScore:setPosition(ws.width/2,550)
        ...
        shareBtn:addTouchEventListener(function(sender, event)
            if event == ccui.TouchEventType.ended then
                G.takeScreenshot()
                share(string.format("我正在玩Filper Block，我分数%s，你也下载来挑战下我吧：http://www.wandoujia.com/apps/org.cocos2dx.FlipBlocks",G.formatTime(best)))
            end 
        end)
